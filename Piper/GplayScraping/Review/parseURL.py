...
import os
import time
from reviewparse import fetchAllReviews
from reviewdulpicateremove import removeDuplicate
...
print 'Review Fetcing Job stated on ' + str(datetime.datetime.now())
...
def storeInCSV(html,path):
	...
	if os.path.isfile(fname) == False:
		...
		#get the last line
		#print "Reading last line of csv"
		a = open(fname,'rb')
		lines = a.readlines()
		last_line = lines[-1]
		#print last_line
		...
		dateStr = words[0]
		lastDate = datetime.datetime.strptime(dateStr, '%m-%d-%Y').date()
		a.close() 
...
def mainJob():
	...
	for line in lines:
		#req = urllib2.Request('https://play.google.com/store/apps/details?id=' + line)
		#try: response = urllib2.urlopen(req)
		#except URLError as e:
		#    print "*****ERROR****** FOR " + e.reason
		#    continue
		...
		lastUpdatedDate = storeInCSV(html,'data/' + line)	
		...
		#htmlFilePath = dateDirPath + '/' + 'app.html'
		...
		uniquefname = "data/" + line +"/uniquereviews.csv"

		if os.path.isfile(uniquefname):
			f = open(uniquefname, 'r')
			lines = f.readlines()
			previousReviewCount = len([l for l in lines if l.strip(' \n') != ''])
			print "Privious reviewcount " + str(previousReviewCount)

		
		allReviews = fetchAllReviews(line,lastUpdatedDate, previousReviewCount)

		print "Todays collected review count " + str(len(allReviews))

		reversedAllReviews = reversed(allReviews)
		allrcfn = 'data/' + line + '/' + 'allreviews.csv'
		allrc = csv.writer(open(allrcfn, "a"))
		reviewFilePath = dateDirPath + '/' + 'review.csv'
		rc = csv.writer(open(reviewFilePath, "w"))

		for onereview in reversedAllReviews:
			words = onereview.split(';')
			rc.writerow(words)
			allrc.writerow(words)

		else:
			print "already we have 200 reviews in our database"
...
print 'Review Fetching Job ended on ' + str(datetime.datetime.now())
